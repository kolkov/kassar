...
// Unauthorized creates a new API error representing an authentication failure (HTTP 401)
func SignupError(err string) *APIError {
...
// NoContent creates a new API error representing a no_content error (HTTP 204)
func NotContent(resource string) *APIError {
	return NewAPIError(http.StatusNoContent, "NO_CONTENT", Params{"resource": resource})
}
